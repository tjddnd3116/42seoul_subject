...
/*   Updated: 2022/06/11 20:25:02 by soum             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_txt_pos(t_point *ray, mlx_texture_t *txt, t_door *door)
{
	...
	if (ray->type == 'N' || ray->type == 'S')
	{
		...
		if (ray->is_close_door && ray->wall_len <= GRID * 2)
...
void	draw_wall(t_point *pnt, t_mlx_data *data, int i)
{
	...
	start_end_xy[5] = draw_pixel_y;
	if (draw_pixel_y >= SCREEN_H)
	{
		start_end_xy[4] = (draw_pixel_y - SCREEN_H) / 2.0;
		start_end_xy[4] = (start_end_xy[4] / draw_pixel_y) * txt->height;
	...
	put_wall_pixel(data, start_end_xy, txt_pos, txt);
}
