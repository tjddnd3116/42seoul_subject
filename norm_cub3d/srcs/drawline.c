...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/29 15:36:32 by soum              #+#    #+#             */
/*   Updated: 2022/06/01 15:05:19 by soum             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fdf_drawline(mlx_image_t *img, t_point *p0, t_point *p1)
{
	...
	if ((int)(p1->x - p0->x) < 0)
		swap_point(&p0, &p1);
	dy = p1->y - p0->y;
	if ((uint32_t)p0->x >= SCREEN_W || (uint32_t)p0->y >= SCREEN_H
		|| (uint32_t)p1->x >= SCREEN_W || (uint32_t)p1->y >= SCREEN_H)
		return ;
