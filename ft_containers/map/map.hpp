...
class map
{
	...
	void					insert(InputIterator first, InputIterator last,
			typename std::enable_if<!std::is_integral<InputIterator>::value>::type* = 0);	// range (3)
	void					erase(iterator position);							// positin (1)
	size_type				erase(const key_type& k);							// single element(2)
	void					erase(iterator first, iterator last);				// range(3)
	void					swap(map& x);
	void					clear();
...
map<Key, T, Compare, Alloc>::map(const key_compare& comp, const allocator_type& alloc) :_comp(comp), _alloc(alloc)
{
